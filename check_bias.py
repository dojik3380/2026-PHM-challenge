...
def main():
    ...
    for fold, held_out in enumerate(tdms_cases, start=1):
        ...
        bias = np.mean(s2_rul - true_rul) / 3600.0 # in hours
        total_bias.append(bias)
        
        direction = "Over-estimating" if bias > 0 else "Under-estimating"
        print(f"{held_out:<15} | Bias: {bias:>6.2f}h | {direction}")
        
    print(f"\nAverage Bias: {np.mean(total_bias):.2f}h")
