...
def main():
    ...
    print("\n" + "="*70)
    print(f"{'Case Name':<15} | {'True RUL (h)':<12} | {'Pred RUL (h)':<12} | {'Error (h)':<10} | {'A_RUL Score':<10}")
    print("-" * 70)
    ...
    for fold, held_out in enumerate(tdms_cases, start=1):
        ...
        # Apply optimal config.py Stage 2 parameters implicitly
        ...
        score = np.mean(asymmetric_rul_score_np(s2_rul, true_rul))
        mae = np.mean(np.abs(s2_rul - true_rul))
        ...
        print(f"{held_out:<15} | Trajectory MAE: {mae_h:>5.1f}h | Score: {score:>7.4f}")

    print("=" * 70)
    print(f"Average A_RUL Score : {np.mean(total_scores):.4f}")
    print(f"Average MAE         : {np.mean(total_maes)/3600.0:.1f} hours")
    print("=" * 70)
